...
template<typename SeqNoType, typename IdType>
struct timespec
PacketRecord<SeqNoType, IdType>::GetPacketSentTime(SeqNoType msg_no) {
  MessageRecord<SeqNoType, IdType>* msg_record = SafeGetMessageRecord(msg_no);
  return msg_record->sent_time;
}

template<typename SeqNoType, typename IdType>
MessageRecord<SeqNoType, IdType>*
PacketRecord<SeqNoType, IdType>::SafeGetMessageRecord(SeqNoType msg_no) {
  typename std::vector<MessageRecord<SeqNoType, IdType> >::iterator
      msg_record_iterator = msg_records_.begin();
  ...
  if (msg_record_iterator == msg_records_.end()) {
    std::cerr << "ERROR: could not find msg no " << msg_no << " for packet "
              << seq_no_ << std::endl;
    exit(-1);
  }
...
PacketRecord<SeqNoType, IdType>::PacketRecord(
    CPacket& packet,
    PacketState initial_packet_state) {
...
void PacketTracker<SeqNoType, IdType>::EnqueuePacket(CPacket& packet) {
  ...
  typename std::unordered_map<SeqNoType,
                              PacketRecord<SeqNoType, IdType>*>::iterator
      packet_record_iter = packet_record_map_.find(seq_no);
  if (packet_record_iter == packet_record_map_.end()) {
    ...
    packet_record_map_.insert(
        std::make_pair(seq_no, new PacketRecord<SeqNoType, IdType>(
            packet, PACKET_STATE_QUEUED)));
    send_queue_.push(seq_no);
  } else {
    std::cerr << "ERROR: Attempted to enqueue packet that already has a record!"
              << std::endl;
...
void PacketTracker<SeqNoType, IdType>::OnPacketSent(CPacket& packet) {
  ...
  typename std::unordered_map<SeqNoType,
      ...
      packet_record_iter = packet_record_map_.find(seq_no);
  if (packet_record_iter == packet_record_map_.end()) {
    std::cerr << "ERROR: Attempted to send packet not already queued!"
              << std::endl;
    std::cerr << "\t seq_no = " << seq_no << std::endl;
    exit(-1);
  } else {
    if (packet_record_iter->second->GetPacketState() == PACKET_STATE_QUEUED) {
      ...
      if (seq_no != retransmittable_seq_no) {
        std::cerr << "ERROR: Attempted to retransmit out of order!"
                  << std::endl;
        std::cerr << "\t seq_no = " << seq_no << std::endl;
        std::cerr << "\t retransmittable_seq_no = " << retransmittable_seq_no
                  << std::endl;
        exit(-1);
      } else {
    ...
    packet_record->UpdateRecord(PACKET_STATE_SENT, packet.m_iMsgNo,
                                MakeNewPacketId(packet));
    sent_queue_.push(packet_record->GetPacketSentTime(packet.m_iMsgNo));
    sent_time_map_.insert(std::make_pair(
        packet_record->GetPacketSentTime(packet.m_iMsgNo), seq_no));
  }
}
...
void PacketTracker<SeqNoType, IdType>::OnPacketAck(SeqNoType seq_no,
  ...
  if (packet_record_iter != packet_record_map_.end()) {
    ...
    // std::cerr << "ERROR: Packet was acked but never recorded as sent!"
    //           << std::endl;
    // std::cerr << "\t seq_no = " << seq_no << std::endl;
    // exit(-1);
  }
...
void PacketTracker<SeqNoType, IdType>::OnPacketLoss(SeqNoType seq_no,
  ...
  if (packet_record_iter != packet_record_map_.end()) {
    ...
    // std::cout << "Added " << seq_no << " to retransmittable queue"
    //           << std::endl;
  } else {
    // std::cerr << "ERROR: Packet was lost but never recorded as sent!"
    //           << std::endl;
    // std::cerr << "\t seq_no = " << seq_no << std::endl;
    // exit(-1);
  }
...
template<typename SeqNoType, typename IdType>
struct timespec PacketTracker<SeqNoType, IdType>::GetPacketSentTime(
    SeqNoType seq_no, SeqNoType msg_no) {
  std::lock_guard<std::mutex> guard(lock_);
  typename std::unordered_map<SeqNoType,
                              PacketRecord<SeqNoType, IdType>*>::iterator
      packet_record_iter = packet_record_map_.find(seq_no);
  if (packet_record_iter == packet_record_map_.end()) {
    ...
    ts.tv_nsec = 0;
    return ts;
    // std::cerr << "ERROR: Attempted to get the sending time of an unrecorded "
    //           << "packet!" << std::endl;
    // std::cerr << "\t seq_no = " << seq_no << std::endl;
    // exit(-1);
  } else {
    struct timespec result =
        packet_record_iter->second->GetPacketSentTime(msg_no);
    return result;
  }
}
...
IdType PacketTracker<SeqNoType, IdType>::GetPacketId(SeqNoType seq_no,
  ...
  typename std::unordered_map<SeqNoType,
      ...
      packet_record_iter = packet_record_map_.find(seq_no);
  if (packet_record_iter == packet_record_map_.end()) {
    // std::cerr << "ERROR: Attempted to get the id of an unrecorded packet!"
    //           << std::endl;
    // std::cerr << "\t seq_no = " << seq_no << std::endl;
    // exit(-1);
    return 0;
...
uint64_t PacketTracker<SeqNoType, IdType>::GetPacketRtt(SeqNoType seq_no,
  ...
  typename std::unordered_map<SeqNoType,
      ...
      packet_record_iter = packet_record_map_.find(seq_no);
  if (packet_record_iter == packet_record_map_.end()) {
    // std::cerr << "ERROR: Attempted to get the rtt of an unrecorded packet!"
    //           << std::endl;
    // std::cerr << "\t seq_no = " << seq_no << std::endl;
    // exit(-1);
    return 0;
...
PacketState PacketTracker<SeqNoType, IdType>::GetPacketState(SeqNoType seq_no) {
  ...
  typename std::unordered_map<SeqNoType,
      ...
      packet_record_iter = packet_record_map_.find(seq_no);
  if (packet_record_iter == packet_record_map_.end()) {
    // std::cerr << "ERROR: Attempted to get the size of an unrecorded packet!"
    //           << std::endl;
    // std::cerr << "\t seq_no = " << seq_no << std::endl;
    // exit(-1);
...
template<typename SeqNoType, typename IdType>
SeqNoType PacketTracker<SeqNoType, IdType>::GetPacketLastMsgNo(
    SeqNoType seq_no) {
  std::lock_guard<std::mutex> guard(lock_);
  typename std::unordered_map<SeqNoType,
                              PacketRecord<SeqNoType, IdType>*>::iterator
...
SeqNoType PacketTracker<SeqNoType, IdType>::GetOldestSentSeqNo() {
  std::lock_guard<std::mutex> guard(lock_);
  typename std::unordered_map<struct timespec, SeqNoType>::iterator
...
bool PacketTracker<SeqNoType, IdType>::HasRetransmittablePackets() {
  std::lock_guard<std::mutex> guard(lock_);
  while (!retransmittable_queue_.empty() &&
         packet_record_map_.find(retransmittable_queue_.top()) ==
             packet_record_map_.end()) {
    retransmittable_queue_.pop();
  }
...
template<typename SeqNoType, typename IdType>
char* PacketTracker<SeqNoType, IdType>::GetPacketPayloadPointer(
    SeqNoType seq_no) {
  lock_.lock();
  typename std::unordered_map<SeqNoType,
                              PacketRecord<SeqNoType, IdType>*>::iterator
      packet_record_iter = packet_record_map_.find(seq_no);
  if (packet_record_iter == packet_record_map_.end()) {
    std::cerr << "ERROR: Attempted to get the payload of an unrecorded packet!"
              << std::endl;
    std::cerr << "\t seq_no = " << seq_no << std::endl;
    exit(-1);
  } else {
